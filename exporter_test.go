...
func TestExporterEmptyValue(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc(
		"/status.html", func(w http.ResponseWriter, r *http.Request) {
			http.ServeFile(w, r, "test_data_empty.html")
		},
	)
	testDataServer := http.Server{
		Handler: mux,
	...
	testContains(t, body, "solar_power_watt 0")
	testContains(t, body, "solar_power_yield_today_kwh 0")
	testContains(t, body, "solar_power_yield_total_kwh 0")
	testServer.Close()
	testDataServer.Close()
